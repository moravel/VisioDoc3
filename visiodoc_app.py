...
def get_system_font(size):
    ...
    try:
        ...
        mf = fm.findfont(fm.FontProperties(family="sans-serif"))
        if mf:
            c.insert(0, mf)
    except ImportError:
        pass
...
# Import pyi_splash for PyInstaller splash screen control (only available in frozen builds)
# Importe pyi_splash pour le contrôle de l'écran de démarrage de PyInstaller (uniquement dans les builds compilés)
try:
    import pyi_splash
except ImportError:
    pyi_splash = None
...
from tooltip import (
    Tooltip,
)  # Import Tooltip class for hover help / Importe la classe Tooltip pour l'aide au survol
from video_stream import (
    VideoStreamThread,
)  # Import VideoStreamThread for camera handling / Importe VideoStreamThread pour la gestion de la caméra
...
class VisioDoc3(tk.Tk):
    ...
    if getattr(sys, "frozen", False):
        # PyInstaller: use the bundled icons directory
        # PyInstaller: utilise le répertoire d'icônes groupé
        ICON_DIR = os.path.join(sys._MEIPASS, "icons")
    else:
        # Development mode: use local icons directory
        # Mode développement: utilise le répertoire d'icônes local
        ICON_DIR = os.path.join(os.path.dirname(__file__), "icons")

    # Configuration file path for camera settings (platform-appropriate)
    # Chemin du fichier de configuration pour les paramètres de la caméra (adapté à la plateforme)
    if sys.platform == "win32":
        # Windows: use AppData/Roaming
        config_dir = os.path.join(
            os.environ.get("APPDATA", os.path.expanduser("~")), "VisioDoc3"
        )
    elif sys.platform == "darwin":
        # macOS: use Application Support
        config_dir = os.path.join(
            os.path.expanduser("~/Library/Application Support/VisioDoc3")
        )
    else:
        # Linux/other: use XDG_CONFIG_HOME or ~/.config
        base = os.environ.get("XDG_CONFIG_HOME", os.path.expanduser("~/.config"))
        config_dir = os.path.join(base, "visiodoc3")
    ...
    def __init__(self):
        ...
        self.video_stream_thread = (
            None  # Thread managing camera capture / Fil gérant la capture de la caméra
        )
        ...
        self.current_annotation_color = (
            0,
            0,
            255,
        )  # Default annotation color (BGR for OpenCV: Blue) / Couleur d'annotation par défaut (BGR pour OpenCV : Bleu)
        ...
        self.tooltips = []  # List to store Tooltip objects for language refresh / Liste pour stocker les objets Tooltip pour le rafraîchissement de langue
        ...
        self.bind(
            "<Control-f>", lambda event: self.set_tool("freedraw")
        )  # Freehand draw tool / Outil de dessin à main levée
        self.bind(
            "<Control-r>", lambda event: self.set_tool("rectangle")
        )  # Rectangle tool / Outil rectangle
        self.bind(
            "<Control-c>", lambda event: self.set_tool("circle")
        )  # Circle tool / Outil cercle
        self.bind(
            "<Control-l>", lambda event: self.set_tool("line")
        )  # Line tool / Outil ligne
        self.bind(
            "<Control-t>", lambda event: self.set_tool("text")
        )  # Text tool / Outil texte
        self.bind(
            "<Control-b>", lambda event: self.set_tool("blur")
        )  # Blur tool / Outil de flou
        self.bind(
            "<Control-a>", lambda event: self.set_tool("arrow")
        )  # Arrow tool / Outil flèche
        self.bind(
            "<Control-h>", lambda event: self.set_tool("highlight")
        )  # Highlight tool / Outil surlignage
        self.bind(
            "<Control-s>", lambda event: self.set_tool("selection")
        )  # Selection tool / Outil de sélection

        self.bind(
            "<Control-Shift-S>", lambda event: self.save_image()
        )  # Save image / Sauvegarder l'image
        self.bind(
            "<Control-e>", lambda event: self.clear_all_annotations()
        )  # Clear all annotations / Effacer toutes les annotations
        self.bind(
            "<Control-z>", lambda event: self.undo_last_annotation()
        )  # Undo last action / Annuler la dernière action
        self.bind(
            "<Control-y>", lambda event: self.redo_last_annotation()
        )  # Redo last undone action / Rétablir la dernière action annulée
        self.bind(
            "<Control-p>", lambda event: self.open_settings_dialog()
        )  # Open settings dialog / Ouvrir la boîte de dialogue des paramètres

        self.bind("<Control-plus>", self.zoom_in)  # Zoom in / Zoom avant
        self.bind(
            "<Control-equal>", self.zoom_in
        )  # Zoom in (alternative for some keyboards) / Zoom avant (alternative pour certains claviers)
        self.bind("<Control-minus>", self.zoom_out)  # Zoom out / Zoom arrière

        self.bind(
            "<Control-k>", lambda event: self.choose_annotation_color()
        )  # Choose annotation color / Choisir la couleur de l'annotation
        self.bind(
            "<Control-i>", lambda event: self.choose_annotation_size()
        )  # Choose annotation size / Choisir la taille de l'annotation
        self.bind(
            "<Control-j>", lambda event: self.flip_horizontal()
        )  # Flip image horizontally / Retourner l'image horizontalement
        self.bind(
            "<Control-u>", lambda event: self.flip_vertical()
        )  # Flip image vertically / Retourner l'image verticalement

        self.bind(
            "<Control-question>", lambda event: self.show_help()
        )  # Show help manual / Afficher le manuel d'aide
        self.bind(
            "<Escape>", lambda event: self.exit_fullscreen()
        )  # Exit fullscreen with ESC / Quitter le plein écran avec ESC
    ...
    def on_language_change(self):
        ...
        if hasattr(self, "compact_sidebar") and self.compact_sidebar:
            self.compact_sidebar.refresh_tooltips()
        self._refresh_classic_tooltips()
    ...
    def _setup_video_display(self):
        ...
        text = self.language_manager.tr("help.loading")
    ...
    def _create_tool_buttons(self):
        ...
        def add_button_and_tooltip(parent, text, image, command, tooltip_text):
            btn = ttk.Button(
                parent,
                text=text,
                image=image,
            ...
            tooltip = Tooltip(parent.winfo_children()[-1], tooltip_text)
            self.tooltips.append(tooltip)

        add_button_and_tooltip(
            self.left_panel,
            lm.tr("tools.freedraw.name"),
            self.icons.get("freedraw"),
            lambda: self.set_tool("freedraw"),
            lm.tr("tools.freedraw.tooltip"),
        )

        add_button_and_tooltip(
            self.left_panel,
            lm.tr("tools.rectangle.name"),
            self.icons.get("rectangle"),
            lambda: self.set_tool("rectangle"),
            lm.tr("tools.rectangle.tooltip"),
        )

        add_button_and_tooltip(
            self.left_panel,
            lm.tr("tools.circle.name"),
            self.icons.get("circle"),
            lambda: self.set_tool("circle"),
            lm.tr("tools.circle.tooltip"),
        )

        add_button_and_tooltip(
            self.left_panel,
            lm.tr("tools.line.name"),
            self.icons.get("line"),
            lambda: self.set_tool("line"),
            lm.tr("tools.line.tooltip"),
        )

        add_button_and_tooltip(
            self.left_panel,
            lm.tr("tools.text.name"),
            self.icons.get("text"),
            lambda: self.set_tool("text"),
            lm.tr("tools.text.tooltip"),
        )

        add_button_and_tooltip(
            self.left_panel,
            lm.tr("tools.blur.name"),
            self.icons.get("blur"),
            lambda: self.set_tool("blur"),
            lm.tr("tools.blur.tooltip"),
        )

        add_button_and_tooltip(
            self.left_panel,
            lm.tr("tools.arrow.name"),
            self.icons.get("arrow"),
            lambda: self.set_tool("arrow"),
            lm.tr("tools.arrow.tooltip"),
        )

        add_button_and_tooltip(
            self.left_panel,
            lm.tr("tools.highlight.name"),
            self.icons.get("highlight"),
            lambda: self.set_tool("highlight"),
            lm.tr("tools.highlight.tooltip"),
        )

        add_button_and_tooltip(
            self.left_panel,
            lm.tr("tools.selection.name"),
            self.icons.get("selection"),
            lambda: self.set_tool("selection"),
            lm.tr("tools.selection.tooltip"),
        )

        add_button_and_tooltip(
            self.left_panel,
            lm.tr("tools.color_picker.name"),
            self.icons.get("color_picker"),
            self.choose_annotation_color,
            lm.tr("tools.color_picker.tooltip"),
        )

        add_button_and_tooltip(
            self.left_panel,
            lm.tr("tools.size_picker.name"),
            self.icons.get("size_picker"),
            self.choose_annotation_size,
            lm.tr("tools.size_picker.tooltip"),
        )

        add_button_and_tooltip(
            self.left_panel,
            lm.tr("view.flip_horizontal"),
            self.icons.get("flip_horizontal"),
            self.flip_horizontal,
            f"{lm.tr('view.flip_horizontal')} (Ctrl+J)",
        )

        add_button_and_tooltip(
            self.left_panel,
            lm.tr("view.flip_vertical"),
            self.icons.get("flip_vertical"),
            self.flip_vertical,
            f"{lm.tr('view.flip_vertical')} (Ctrl+U)",
        )
        ...
        ttk.Label(
            zoom_frame,
            text=lm.tr("view.zoom_in").replace("+", ""),
            style="White.TLabel",
        ...
        tooltip_in = Tooltip(zoom_in_btn, f"{lm.tr('view.zoom_in')} (Ctrl++)")
        self.tooltips.append(tooltip_in)
        zoom_out_btn = ttk.Button(
            zoom_frame, text="-", style="Modern.TButton", command=self.zoom_out, width=3
        )
        zoom_out_btn.pack(side=tk.LEFT)
        tooltip_out = Tooltip(zoom_out_btn, f"{lm.tr('view.zoom_out')} (Ctrl+-)")
        self.tooltips.append(tooltip_out)

    def _refresh_classic_tooltips(self):
        """Refresh tooltip texts in classic layout when language changes."""
        if not self.use_hybrid_layout:
            lm = self.language_manager
            for i, tooltip in enumerate(self.tooltips):
                if i == 0:
                    tooltip.set_text(lm.tr("tools.freedraw.tooltip"))
                elif i == 1:
                    tooltip.set_text(lm.tr("tools.rectangle.tooltip"))
                elif i == 2:
                    tooltip.set_text(lm.tr("tools.circle.tooltip"))
                elif i == 3:
                    tooltip.set_text(lm.tr("tools.line.tooltip"))
                elif i == 4:
                    tooltip.set_text(lm.tr("tools.text.tooltip"))
                elif i == 5:
                    tooltip.set_text(lm.tr("tools.blur.tooltip"))
                elif i == 6:
                    tooltip.set_text(lm.tr("tools.arrow.tooltip"))
                elif i == 7:
                    tooltip.set_text(lm.tr("tools.highlight.tooltip"))
                elif i == 8:
                    tooltip.set_text(lm.tr("tools.selection.tooltip"))
                elif i == 9:
                    tooltip.set_text(lm.tr("tools.color_picker.tooltip"))
                elif i == 10:
                    tooltip.set_text(lm.tr("tools.size_picker.tooltip"))
                elif i == 11:
                    tooltip.set_text(f"{lm.tr('view.flip_horizontal')} (Ctrl+J)")
                elif i == 12:
                    tooltip.set_text(f"{lm.tr('view.flip_vertical')} (Ctrl+U)")
                elif i == 13:
                    tooltip.set_text(f"{lm.tr('view.zoom_in')} (Ctrl++)")
                elif i == 14:
                    tooltip.set_text(f"{lm.tr('view.zoom_out')} (Ctrl+-)")
                elif i == 15:
                    tooltip.set_text(f"{lm.tr('menus.file.open')} file (Ctrl+O)")
                elif i == 16:
                    tooltip.set_text(lm.tr("menus.file.close"))
                elif i == 17:
                    tooltip.set_text(f"{lm.tr('actions.save')} (Ctrl+Shift+S)")
                elif i == 18:
                    tooltip.set_text(f"{lm.tr('actions.clear')} (Ctrl+E)")
                elif i == 19:
                    tooltip.set_text(f"{lm.tr('actions.undo')} (Ctrl+Z)")
                elif i == 20:
                    tooltip.set_text(f"{lm.tr('actions.redo')} (Ctrl+Y)")
                elif i == 21:
                    tooltip.set_text(f"{lm.tr('actions.delete')} (Suppr)")
                elif i == 22:
                    tooltip.set_text(f"{lm.tr('view.settings')} (Ctrl+P)")
                elif i == 23:
                    tooltip.set_text(f"{lm.tr('actions.help')} (Ctrl+?)")
                elif i == 24:
                    tooltip.set_text(f"{lm.tr('view.fullscreen')} (F11)")
    ...
    def _create_right_panel(self):
        ...
        add_button_and_tooltip(
            self.right_panel,
            lm.tr("menus.file.open"),
            self.icons.get("open_file"),
            self.open_file,
            f"{lm.tr('menus.file.open')} file (Ctrl+O)",
        )

        add_button_and_tooltip(
            self.right_panel,
            lm.tr("menus.file.close"),
            self.icons.get("close_file"),
            self.close_file,
            lm.tr("dialogs.close_file"),
        )
        ...
        add_button_and_tooltip(
            ...
            self.icons.get("save"),
            self.save_image,
            f"{lm.tr('actions.save')} (Ctrl+Shift+S)",
        )

        add_button_and_tooltip(
            self.right_panel,
            lm.tr("actions.clear"),
            self.icons.get("clear"),
            self.clear_all_annotations,
            f"{lm.tr('actions.clear')} (Ctrl+E)",
        )
        ...
        add_button_and_tooltip(
            ...
            f"{lm.tr('actions.redo')} (Ctrl+Y)",
        )

        add_button_and_tooltip(
            self.right_panel,
            lm.tr("actions.delete"),
            self.icons.get("delete"),
            self.delete_selected_annotation,
            f"{lm.tr('actions.delete')} (Suppr)",
        )
        ...
        add_button_and_tooltip(
            ...
            self.icons.get("help"),
            self.show_help,
            f"{lm.tr('actions.help')} (Ctrl+?)",
        )

        add_button_and_tooltip(
            self.right_panel,
            lm.tr("view.fullscreen"),
            self.icons.get("fullscreen"),
            self.toggle_fullscreen,
            f"{lm.tr('view.fullscreen')} (F11)",
        )
    ...
    def _setup_controls(self):
        ...
        self.camera_menu_placeholder = ttk.Label(
            self.camera_selection_frame, text=self.language_manager.tr("help.loading")
        )
    ...
    def show_help(self):
        ...
        help_dialog.title(self.language_manager.tr("help.title"))
        help_dialog.transient(
            self
        )  # Make it transient to the main window / Le rend transitoire à la fenêtre principale
        ...
        text_widget.insert(tk.END, self.language_manager.tr("help.manual"))
        text_widget.config(
            state="disabled"
        )  # Make it read-only / Le rend en lecture seule
        ...
        ttk.Button(
            help_dialog,
            text=self.language_manager.tr("help.close"),
        ...
        x = (
            self.winfo_x()
            + (self.winfo_width() // 2)
            - (help_dialog.winfo_width() // 2)
        )
        y = (
            self.winfo_y()
            + (self.winfo_height() // 2)
            - (help_dialog.winfo_height() // 2)
        )
        help_dialog.geometry(
            f"{help_dialog.winfo_width()}x{help_dialog.winfo_height()}+{x}+{y}"
        )
    ...
    def _initialize_camera(self):
        ...
        if cached_camera_index is not None:
            ...
            if cap.isOpened():
                ...
                self.camera_options.append(
                    (f"Webcam {cached_camera_index}", cached_camera_index)
                )
                cap.release()
                ...
                self.after(
                    0,
                    lambda: self.update_camera_menu(
                        initial_camera_index=cached_camera_index
                    ),
                )
                return
            else:
                print(
                    f"Cached camera {cached_camera_index} not available. Scanning for cameras..."
                )
    ...
    def _check_camera(self, index, results_list, initial_camera_found):
        ...
        if cap.isOpened():
            ...
            if not initial_camera_found[0]:
                self.after(
                    0,
                    lambda: self.start_video_stream(
                        index, self.current_resolution[0], self.current_resolution[1]
                    ),
                )
    ...
    def populate_cameras(self):
        ...
        """
        if (
            platform.system() == "Windows" and DSShow is not None
        ):  # Use pygrabber on Windows if available / Utilise pygrabber sous Windows si disponible
            try:
                devices = DSShow.get_input_devices()
                for i, device_name in enumerate(devices):
                    self.camera_options.append((device_name, i))
                self.after(
                    0, self.update_camera_menu
                )  # Update UI on main thread / Met à jour l'interface utilisateur sur le fil principal
    ...
    def _populate_cameras_linux(self):
        ...
        threads = []
        found_cameras = []
        initial_camera_found = [
            False
        ]  # Use a mutable list to pass by reference / Utilise une liste mutable pour passer par référence
        for i in range(
            10
        ):  # Check up to 10 potential camera indices / Vérifie jusqu'à 10 indices de caméra potentiels
            thread = threading.Thread(
                target=self._check_camera,
                args=(i, found_cameras, initial_camera_found),
                daemon=True,
            )
    ...
    def start_video_stream(self, camera_index, width, height):
        ...
        if self.video_stream_thread and self.video_stream_thread.is_alive():
            ...
            self.video_stream_thread.join(
                timeout=5.0
            )  # Wait for the thread to terminate / Attend que le fil se termine
            if self.video_stream_thread.is_alive():
                print("Warning: Video stream thread did not terminate within timeout")
    ...
    def update_display(self):
        ...
        if (
            self.file_mode
        ):  # If in file viewing mode / Si en mode d'affichage de fichier
            if self.loaded_image:
                self.display_image(self.loaded_image)
        elif (
            self.video_stream_thread and self.video_stream_thread.is_alive()
        ):  # If in webcam mode and thread is active / Si en mode webcam et que le fil est actif
    ...
    def display_image(self, pil_image):
        ...
        if self.selected_annotation:
            ...
            if bbox:
                ...
                cv2.rectangle(
                    display_image_cv, p1, p2, (0, 255, 0), 2, cv2.LINE_AA
                )  # Green bounding box / Boîte englobante verte
        ...
        # Dessine la boîte englobante pour l'annotation survolée (si aucune annotation n'est sélectionnée)
            ...
            if bbox:
                ...
                p2 = (int(bbox[2]), int(bbox[3]))
                cv2.rectangle(
                    display_image_cv, p1, p2, (255, 165, 0), 2, cv2.LINE_AA
                )  # Orange for hover / Orange pour le survol
        ...
        if (
            ...
            and self.end_point
            and self.current_tool != "selection"
        ):
            if self.current_tool == "line":
                temp_annotation = LineAnnotation(
                    self.start_point,
                    self.end_point,
                ...
                temp_annotation = RectangleAnnotation(
                    self.start_point,
                    self.end_point,
                    color=self.current_annotation_color,
                    thickness=self.current_annotation_thickness,
                ...
                temp_annotation = CircleAnnotation(
                    (center_x, center_y),
                    radius,
                    color=self.current_annotation_color,
                    thickness=self.current_annotation_thickness,
                ...
                self.current_tool == "freedraw" and self.current_freedraw_points
            ):  # For freehand, draw the current path / Pour le dessin à main levée, dessine le chemin actuel
                temp_annotation = FreeDrawAnnotation(
                ...
                cv2.rectangle(
                    overlay, (x1, y1), (x2, y2), (255, 255, 0), -1
                )  # Draw a temporary yellow filled rectangle / Dessine un rectangle rempli jaune temporaire
                alpha = 0.3  # Opacity for the temporary blur preview / Opacité pour l'aperçu de flou temporaire
                display_image_cv = cv2.addWeighted(
                    overlay, alpha, display_image_cv, 1 - alpha, 0
                )
            elif self.current_tool == "arrow":
                temp_annotation = ArrowAnnotation(
                    self.start_point,
                    self.end_point,
                    color=(0, 0, 255),
                    thickness=self.current_annotation_thickness,
                ...
                x2, y2 = self.end_point
                cv2.rectangle(
                    overlay, (x1, y1), (x2, y2), self.current_annotation_color, -1
                )
        ...
        final_annotated_pil = Image.fromarray(
            cv2.cvtColor(display_image_cv, cv2.COLOR_BGR2RGB)
        )
        ...
        scaled_image = final_annotated_pil.resize(
        ...
        final_view = Image.new(
            "RGB", (label_width, label_height), (200, 200, 200)
        )  # Grey background for empty areas / Fond gris pour les zones vides
        ...
        final_view.paste(
            scaled_image, (paste_x, paste_y)
        )  # Paste the scaled image onto the final view canvas / Colle l'image mise à l'échelle sur le canevas de la vue finale
    ...
    def on_closing(self):
        ...
        if (
            self.video_stream_thread
        ):  # If a video stream is active / Si un flux vidéo est actif
            self.video_stream_thread.stop()  # Signal the thread to stop / Signale au fil de s'arrêter
            self.video_stream_thread.join(
                timeout=5.0
            )  # Wait for the thread to finish / Attend que le fil se termine
            if self.video_stream_thread.is_alive():
                print(
                    "Warning: Video stream thread did not terminate within timeout in on_closing"
                )
    ...
    def choose_annotation_color(self):
        ...
        if color_code[
            1
        ]:  # If a color was selected (color_code[1] is the hex string) / Si une couleur a été sélectionnée (color_code[1] est la chaîne hexadécimale)
            rgb_color = color_code[
                0
            ]  # RGB tuple (e.g., (255, 0, 0) for red) / Tuple RGB (par exemple, (255, 0, 0) pour le rouge)
    ...
    def choose_annotation_size(self):
        ...
        thickness_slider.set(
            self.current_annotation_thickness
        )  # Set initial value / Définit la valeur initiale
    ...
    def set_tool(self, tool_name):
        ...
        if tool_name == "text":
            ...
            self.hovered_annotation = (
                None  # Clear hovered annotation / Efface l'annotation survolée
            )
    ...
    def save_image(self):
        ...
        """
        if hasattr(
            self, "view_for_saving"
        ):  # Ensure there is an image to save / S'assure qu'il y a une image à sauvegarder
            ...
            if file_path:  # If a file path was selected / Si un chemin de fichier a été sélectionné
                ...
                for desc, pattern in [
                    ...
                    if file_path.lower().endswith(
                        pattern[1:]
                    ):  # Check if path ends with .png or .pdf
                ...
                try:
                    if ext.lower() == ".pdf":
                        ...
                        img_to_save.convert("RGB").save(
                            final_filename, "PDF", resolution=100.0
                        )
                    else:
                    ...
                    messagebox.showerror(
                        ...
                        f"Impossible de sauvegarder l'image :\n{e}",
                    )
        else:
    ...
    def _convert_event_to_original_coords(self, event):
        ...
        if self.file_mode:
            ...
            self.video_stream_thread
            and self.video_stream_thread.get_frame() is not None
        ):
            original_height, original_width, _ = (
                self.video_stream_thread.get_frame().shape
            )
        else:
            return (0, 0)
        ...
        if self.current_tool == "selection":
            ...
            if self.selected_annotation:  # If an annotation is already selected / Si une annotation est déjà sélectionnée
                ...
                for handle_name, handle_pos in handles.items():
                    ...
                    if (
                        abs(handle_pos[0] - click_point[0]) < 15 / self.zoom_level
                        and abs(handle_pos[1] - click_point[1]) < 15 / self.zoom_level
                    ):
            ...
            if (
                newly_selected
            ):  # If an annotation was clicked / Si une annotation a été cliquée
            ...
            entered_text = (
                self.get_text_input()
            )  # Open text input dialog / Ouvre la boîte de dialogue de saisie de texte
            if entered_text:  # If text was entered / Si du texte a été saisi
                ...
                self.set_tool(
                    "selection"
                )  # Switch to selection tool after adding text / Passe à l'outil de sélection après avoir ajouté du texte
        elif (
            self.current_tool != "none"
        ):  # For other drawing tools / Pour les autres outils de dessin
            self.drawing = True
    ...
    def on_mouse_drag(self, event):
        ...
        if (
            self.current_tool == "selection" and self.selected_annotation
        ):  # If selection tool is active and an annotation is selected / Si l'outil de sélection est actif et qu'une annotation est sélectionnée
            ...
            if self.resize_handle:  # If a resize handle is being dragged / Si une poignée de redimensionnement est glissée
                self.selected_annotation.resize(
                    self.resize_handle,
                    current_point,
                    self.initial_drag_point_for_resize,
                )
            else:
                self.selected_annotation.move(
                    dx, dy
                )  # Move the selected annotation / Déplace l'annotation sélectionnée
            ...
            if self.current_tool == "freedraw":
                self.current_freedraw_points.append(
                    self.end_point
                )  # Add points for freehand drawing / Ajoute des points pour le dessin à main levée
    ...
    def on_mouse_up(self, event):
        ...
        self.resize_handle = (
            None  # Clear resize handle / Efface la poignée de redimensionnement
        )

        if (
            self.current_tool != "selection"
        ):  # If a drawing tool was active / Si un outil de dessin était actif
            final_point = self._convert_event_to_original_coords(event)
            self.end_point = final_point

            if (
                self.start_point == self.end_point
            ):  # Ignore zero-size shapes (e.g., simple clicks) / Ignore les formes de taille nulle (par exemple, les simples clics)
                return
            ...
            if self.current_tool == "line":
                ...
                if (
                    radius > 0
                ):  # Only add if radius is positive / Ajoute uniquement si le rayon est positif
                    self.annotations.append(
                        CircleAnnotation(
                            (center_x, center_y),
                            radius,
                ...
                if self.current_freedraw_points:  # Only add if points exist / Ajoute uniquement si des points existent
                    self.annotations.append(
                        FreeDrawAnnotation(
                            list(self.current_freedraw_points),
                            color=self.current_annotation_color,
                            thickness=self.current_annotation_thickness,
                        )
                    )
                ...
                self.annotations.append(
                    BlurAnnotation(self.start_point, self.end_point)
                )
            elif self.current_tool == "arrow":
                self.annotations.append(
                    ArrowAnnotation(
    ...
    def on_mouse_move(self, event):
        ...
        """
        if (
            self.current_tool == "selection" and not self.drawing
        ):  # Only in selection mode and not currently dragging / Uniquement en mode sélection et non en cours de glissement
            mouse_point = self._convert_event_to_original_coords(event)
            self.hovered_annotation = (
                None  # Reset hovered annotation / Réinitialise l'annotation survolée
            )
    ...
    def zoom_in(self, event=None):
        ...
        self.zoom(
            1.1, event.x, event.y
        )  # Zoom in by a factor of 1.1 / Zoom avant d'un facteur de 1.1
    ...
    def zoom_out(self, event=None):
        ...
        self.zoom(
            0.9, event.x, event.y
        )  # Zoom out by a factor of 0.9 / Zoom arrière d'un facteur de 0.9
    ...
    def on_mouse_wheel(self, event):
        ...
        ):  # Scroll down (zoom out) / Défilement vers le bas (zoom arrière)
            self.zoom(0.9, event.x, event.y)
        elif (
            event.num == 4 or event.delta > 0
        ):  # Scroll up (zoom in) / Défilement vers le haut (zoom avant)
    ...
    def clamp_offsets(self):
        ...
        ):  # If image is smaller than label, center it / Si l'image est plus petite que l'étiquette, la centre
            self.view_offset_x = (scaled_width - label_width) / 2
        else:
            max_offset_x = scaled_width - label_width
            self.view_offset_x = max(
                0, min(self.view_offset_x, max_offset_x)
            )  # Ensure offset is between 0 and max_offset_x / S'assure que le décalage est entre 0 et max_offset_x
        ...
        if (
            ...
            self.view_offset_y = max(
                0, min(self.view_offset_y, max_offset_y)
            )  # Ensure offset is between 0 and max_offset_y / S'assure que le décalage est entre 0 et max_offset_y
    ...
    def get_text_input(self):
        ...
        self.text_input_dialog.transient(
            self
        )  # Make it a transient window of the main app / Le rend transitoire à la fenêtre principale de l'application
        ...
        ttk.Button(
            self.text_input_dialog, text="OK", command=self.process_text_input
        ).pack(pady=5)
        ...
        x = (
            self.winfo_x()
            + (self.winfo_width() // 2)
            - (self.text_input_dialog.winfo_width() // 2)
        )
        y = (
            self.winfo_y()
            + (self.winfo_height() // 2)
            - (self.text_input_dialog.winfo_height() // 2)
        )
        self.text_input_dialog.geometry(
            f"{self.text_input_dialog.winfo_width()}x{self.text_input_dialog.winfo_height()}+{x}+{y}"
        )

        self.wait_window(
            self.text_input_dialog
        )  # Wait for the dialog to close before returning / Attend que la boîte de dialogue se ferme avant de retourner
    ...
    def undo_last_annotation(self):
        ...
        if self.annotations:  # Only if there are annotations to undo / Uniquement s'il y a des annotations à annuler
            ...
            )  # Remove the last annotation / Supprime la dernière annotation
            self.redo_stack.append(
                last_annotation
            )  # Add it to the redo stack / L'ajoute à la pile de rétablissement
    ...
    def redo_last_annotation(self):
        ...
        if self.redo_stack:  # Only if there are actions to redo / Uniquement s'il y a des actions à rétablir
            ...
            self.annotations.append(
                last_redone
            )  # Add it back to active annotations / Le rajoute aux annotations actives
    ...
    def delete_selected_annotation(self, event=None):
        ...
        if self.selected_annotation:  # If an annotation is explicitly selected / Si une annotation est explicitement sélectionnée
            ...
            self.hovered_annotation = (
                None  # Clear hover state / Efface l'état de survol
            )
    ...
    def open_settings_dialog(self):
        ...
        self.brightness_slider.set(
            self.video_stream_thread.cap.get(cv2.CAP_PROP_BRIGHTNESS)
        ...
        ttk.Label(
            settings_dialog, text="Contraste:", background="white", style="White.TLabel"
        ).pack(pady=5)
        ...
        self.contrast_slider.set(
            self.video_stream_thread.cap.get(cv2.CAP_PROP_CONTRAST)
            if self.video_stream_thread and self.video_stream_thread.cap
            else 128
        )
        ...
        ttk.Label(settings_dialog, text="Résolution:", style="White.TLabel").pack(
            pady=5
        )
        ...
        self.resolutions = [
            ...
            "1280x720",
            "1920x1080",
        ]  # Common resolutions / Résolutions courantes
        self.resolution_var.set(
            f"{self.current_resolution[0]}x{self.current_resolution[1]}"
        )  # Set current resolution as default / Définit la résolution actuelle comme valeur par défaut
        ...
        ttk.Button(
            settings_dialog, text="Fermer", command=settings_dialog.destroy
        ).pack(pady=10)
    ...
    def set_resolution(self, resolution_str):
        ...
        if (
            ...
            self.start_video_stream(
                self.video_stream_thread.camera_index, width, height
            )
    ...
    def flip_horizontal(self):
        ...
        """
        if (
            self.file_mode and self.loaded_image
        ):  # If in file mode, flip the loaded image / Si en mode fichier, retourne l'image chargée
            self.loaded_image = self.loaded_image.transpose(Image.FLIP_LEFT_RIGHT)
        elif (
            not self.file_mode
            and self.video_stream_thread
            and self.video_stream_thread.is_alive()
        ):  # If in webcam mode, tell the thread to flip / Si en mode webcam, indique au fil de retourner
    ...
    def toggle_fullscreen(self):
        """
        Toggles fullscreen mode for the video display area.
        Active/désactive le mode plein écran pour la zone d'affichage vidéo.
        """
    ...
    def exit_fullscreen(self):
        ...
        Quitte le mode plein écran s'il est actif.
        """
        if self.is_fullscreen:
    ...
    def open_file(self):
        ...
        if (
            not file_path
        ):  # If no file was selected / Si aucun fichier n'a été sélectionné
            return
        ...
        if file_path.lower().endswith(
            ".pdf"
        ):  # If it's a PDF file / Si c'est un fichier PDF
            self.pdf_document = fitz.open(
                file_path
            )  # Open PDF document / Ouvre le document PDF
            self.pdf_pages = [
                page for page in self.pdf_document
            ]  # Get all pages / Obtient toutes les pages
            self.current_pdf_page = (
                0  # Start at the first page / Commence à la première page
            )
            self.load_pdf_page(
                self.current_pdf_page
            )  # Load and display the first page / Charge et affiche la première page
            ...
            try:
                self.loaded_image = Image.open(file_path).convert(
                    "RGB"
                )  # Open and convert image to RGB / Ouvre et convertit l'image en RGB
                self.display_image(self.loaded_image)
            except Exception as e:
                messagebox.showerror(
                    "Erreur d'ouverture", f"Impossible d'ouvrir le fichier image:\n{e}"
                )
    ...
    def close_file(self):
        ...
        if (
            self.pdf_document
        ):  # If a PDF was open, close it / Si un PDF était ouvert, le ferme
            self.pdf_document.close()
        self.pdf_document = None
        self.pdf_pages = []
        self.current_pdf_page = 0
        self.hide_pdf_nav()  # Hide PDF navigation buttons / Masque les boutons de navigation PDF
        self.camera_selection_frame.pack(
            side=tk.LEFT, padx=10
        )  # Show camera selection UI / Affiche l'interface utilisateur de sélection de la caméra
    ...
    def load_pdf_page(self, page_number):
        ...
        Args:
            page_number (int): The 0-based index of the PDF page to load.                                  L\'index (base 0) de la page PDF à charger.
        """
        if (
            self.pdf_document and 0 <= page_number < len(self.pdf_pages)
        ):  # Ensure document is open and page number is valid / S'assure que le document est ouvert et que le numéro de page est valide
            page = self.pdf_pages[page_number]
            pix = page.get_pixmap()  # Render page to pixmap / Rend la page en pixmap
            self.loaded_image = Image.frombytes(
                "RGB", [pix.width, pix.height], pix.samples
            )  # Convert pixmap to PIL Image / Convertit le pixmap en image PIL
            self.current_pdf_page = page_number
            self.display_image(
                self.loaded_image
            )  # Display the loaded page / Affiche la page chargée
            self.page_label.config(
                text=f"Page {self.current_pdf_page + 1} / {len(self.pdf_pages)}"
            )  # Update page number display / Met à jour l'affichage du numéro de page
    ...
    def next_pdf_page(self):
        ...
        """
        if (
            self.current_pdf_page < len(self.pdf_pages) - 1
        ):  # Check if not on the last page / Vérifie si ce n'est pas la dernière page
    ...
    def prev_pdf_page(self):
        ...
        if (
            self.current_pdf_page > 0
        ):  # Check if not on the first page / Vérifie si ce n'est pas la première page
            self.load_pdf_page(self.current_pdf_page - 1)
